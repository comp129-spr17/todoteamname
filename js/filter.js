function filter(){
	$.post('/php/get_player_data.php', $('form').serializeArray(), function(players) {
        ...
        if (!jQuery.isEmptyObject(players)) {
            ...
            var empty_row ='<tr id="found-player"><td style=width:1%></td>';
            var empty_data = '<td style=width:15%><strong>No Results Found</strong></td><td style=width:10%></td><td style=width:10%></td><td style=width:10%></td><td style=width:10%></td><td style=width:10%></td><td style=width:10%></td><td style=width:5%></td><td style=width:5%></td><td style=width:5%></td><td style=width:5%></td>';
            var empty_end ='<\tr>';
            $('#results-table').append(empty_row + empty_data + empty_end);
        }
    }
    ,
    'json');
